...
const GuidesPage = () => {
  useEffect(() => {
    document.title = "PC Building Guides";
  }, []);
  ...
  const renderGuideCard = (guide) => (
    <div className="guide-card" key={guide.title}>
      <div className={`level-tag ${guide.level.toLowerCase()}`}>
        {guide.level}
      </div>
      <h3>{guide.title}</h3>
      ...
      <div className="tag-list">
        {guide.tags.map((tag, i) => (
          <span key={i} className="guide-tag">
            {tag}
          </span>
        ))}
      </div>
      <button className="open-guide-btn" onClick={() => openGuideDetail(guide)}>
        More Info
      </button>
    </div>
  );
      ...
      <main className="guides-main">
        <section className="guides-header">
          ...
          <p className="subtitle">
            Explore our guides to build your ultimate PC
          </p>
          <input
            type="text"
            id="searchInput"
        ...
        {levels.map((level) => (
          <section key={level} className="guide-section">
            ...
            <div className="guide-grid">
              {categorizedGuides[level]
                ?.slice(0, displayLimits[level])
                .map(renderGuideCard)}
            </div>
